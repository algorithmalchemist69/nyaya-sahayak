...
# MAGIC
# MAGIC Interactive demo of both tasks directly inside a Databricks notebook.
# MAGIC
# MAGIC **Flow:**
# MAGIC ```
# MAGIC User input (widget)
# MAGIC     ──► FAISS retrieval (BNS sections)
# MAGIC     ──► Llama 3.1 8B via Databricks Foundation Model API (open-source)
...
# Open-source model served via Databricks Foundation Model APIs
# No external API key needed — uses your Databricks workspace token
...
# Databricks Foundation Model API client — uses workspace token automatically
llm_client = get_deploy_client("databricks")

print("Resources loaded.")
print(f"LLM: {LLM_ENDPOINT} (Meta Llama 3.1 8B — open-source)")
...
# MAGIC %md ## Task 1 — "Explain Like I'm 15" Legal Simplifier
...
dbutils.widgets.text(
    ...
    "Paste BNS / Constitution text here"
)
...
<div style="font-family:sans-serif; max-width:700px; padding:20px;
  ...
  <h3 style="color:#1976d2;">📖 Simple Explanation</h3>
  <p style="font-size:13px;color:#555;">Original text: <em>{legal_text[:120]}…</em></p>
  <hr/>
...
sections = retrieve(incident, top_k=top_k)
...
guidance = call_llm(
    system_prompt=(
        ...
        "Given an incident, identify the offense type, list relevant BNS sections, "
        "explain why each applies in plain language, and advise what to do next. "
        "Be empathetic and clear. Avoid excessive jargon."
    ),
    user_prompt=(
        f"Incident: {incident}\n\n"
...
<div style="font-family:sans-serif; max-width:800px;">
  ...
  <div style="background:#e8f5e9; padding:20px; border-left:4px solid #388e3c; border-radius:6px;">
    <h3 style="color:#388e3c; margin-top:0;">🚨 FIR Guidance</h3>
